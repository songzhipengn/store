...
def Withdrawmoney():
    ...
    if accountnumber in bank:
        if password in bank[accountnumber]["password"]:
           ...
           if takemoney < 0:
                takemoney = 0
           if takemoney <= bank[accountnumber]["money"]:
...
def Transferaccount():
    ...
    if accountnumber in bank:
        if password in bank[accountnumber]["password"]:
            Turninaccount = input("请输入您的转入账号")
            if Turninaccount in bank:
                Turnoutmoney = input("请输入您转账金额")
                Turnoutmoney = int(Turnoutmoney)
                if Turnoutmoney < 0:
                    Turnoutmoney = 0
                if Turnoutmoney <= bank[accountnumber]["money"]:
                    print("是否转账？")
                    a = input("请输入是或否")
                    if a == "是" :
...
def makeenquiries():
    ...
    if accountnumber in bank:
        ...
        ------------个人信息------------
                    ...
                    当前账户的开户行：%s 
        '''
             print(info % (accountnumber,bank[accountnumber]["account"],password,bank[accountnumber]["country"],bank[accountnumber]["province"],bank[accountnumber]["street"],bank[accountnumber]["door"],bank[accountnumber]["money"],bank_name))
        else:
            print("密码错误！")
    else:
        print("用户名错误！")
